...
class AntigenicMapApi
{
    ...
    download_error_lines(args) {
        this._download({command: Object.assign({C: "download_error_lines", id: this.source_id, format: "csv"}, args), blob_type: "application/octet-stream"});
    }
...
function show_point_info(dispatcher, point, invoking_node) {
    ...
    const make_window = (message, invoking_node, title, point, content_filler) => {
        ...
        if (win.content().height() > max_win_content_height)
            win.content().css("height", "" + max_win_content_height + "px");
    };
